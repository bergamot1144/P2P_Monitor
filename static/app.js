...
function positionDropdownMenu(ddId) {
    const root = document.getElementById(ddId); if (!root) return;
...
function attachDropdownRepositioning(ddId) {
    ...
    const obs = new MutationObserver(() => { if (!root.classList.contains('open')) { window.removeEventListener('resize', handler); window.removeEventListener('scroll', handler, true); obs.disconnect(); } });
...
function mdropToggle(ddId) {
    ...
    // ВАЖНО: поднять карточку над соседями
...
function closeAndClear(ddId) {
    ...
    // ВАЖНО: опустить карточку обратно
    const card = dd.closest('.card');
...
function onSearchInput(ddId) { const input = document.getElementById(ddId + '_search'); const val = (input?.value || '').toLowerCase().trim(); searchState[ddId] = val; renderDropdownOptions(ddId); positionDropdownMenu(ddId); }
...
function renderDropdownOptions(ddId) {
    ...
    const grid = document.getElementById(isBin ? 'dd_binance_grid' : 'dd_bybit_grid'); grid.innerHTML = '';
    items.forEach(it => {
        ...
        btn.addEventListener('click', () => {
            ...
            const cnt = document.getElementById(isBin ? 'dd_binance_count' : 'dd_bybit_count'); cnt.textContent = String(temp.size);
        });
...
function updateCounters() { document.getElementById('dd_binance_count').textContent = String(selectedBinance.size); document.getElementById('dd_bybit_count').textContent = String(selectedBybit.size); }
function wireDropdown(ddId) {
    ...
    menu.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.js-reset')) {
            ...
            const cnt = document.getElementById(ddId === 'dd_binance' ? 'dd_binance_count' : 'dd_bybit_count'); if (cnt) cnt.textContent = '0'; return;
        }
    });
    menu.__wired = true;
}
...
async function loadBinancePaytypes() {
    ...
    const merch = document.getElementById('merchant_binance')?.checked ? 'true' : 'false';
    ...
    } catch { binanceItems = []; selectedBinance.clear(); updateCounters(); return []; }
}
...
function paramsFromUI() {
    return {
        ...
        merchant_binance: document.getElementById('merchant_binance')?.checked ? 'true' : 'false',
        paytypes_binance: [...selectedBinance].join(','),
        verified_bybit: document.getElementById('verified_bybit')?.checked ? 'true' : 'false',
...
async function loadBinance() {
    ...
    try {
        ...
        const e = document.getElementById('binance_error'); const ok = document.getElementById('binance_status');
        if (!data.ok) {
            ...
            (data.items || []).forEach((it, i) => {
                ...
                tr.innerHTML = `<td>${i + 1}</td><td>${it.name || '-'}</td><td>${fmt(it.price)}</td><td>${it.volume ?? '-'}</td><td>${it.min ?? '-'}</td><td>${it.max ?? '-'}</td>`; tb.appendChild(tr);
            });
            lastBinanceAvg = next;
        }
    } catch {
        const e = document.getElementById('binance_error'); e.style.display = ''; e.textContent = 'Ошибка сети';
...
async function loadBybit() {
    ...
    try {
        const res = await fetch(url); const data = await res.json();
        const e = document.getElementById('bybit_error'); const ok = document.getElementById('bybit_status');
        if (!data.ok) {
            e.style.display = ''; e.textContent = 'Ошибка: ' + (data.error || 'unknown'); ok.style.display = 'none';
        ...
        const e = document.getElementById('bybit_error'); e.style.display = ''; e.textContent = 'Ошибка сети';
...
async function fillXeCodes() {
    try {
        ...
        if (!xe_to.value) xe_to.value = document.getElementById('fiat').value || 'UAH';
        if (!gf_from.value) gf_from.value = 'USD';
        if (!gf_to.value) gf_to.value = document.getElementById('fiat').value || 'UAH';
    } catch { }
}
...
function applyXE() { refreshXENow(); const pr = currentXePair(); if (pr) history.replaceState(null, '', '?' + new URLSearchParams({ ...Object.fromEntries(new URLSearchParams(location.search)), xe_from: pr.from, xe_to: pr.to })); }
async function loadXE() {
    ...
    try {
        ...
        if (!js.ok) { err.style.display = ''; err.textContent = 'Ошибка XE: ' + (js.error || 'unknown'); xe_price.textContent = '—'; xe_ts.textContent = '—'; xe_src.textContent = '—'; xe_link.href = '#'; xe_spread_bin.textContent = '—'; xe_spread_byb.textContent = '—'; window.__lastXePrice = null; }
        else {
            err.style.display = 'none'; const d = js.data; const next = d.price; setAnimatedText(xe_price, fmtSmart(next) + ' ' + pr.to, window.__lastXePrice, next);
            xe_ts.textContent = 'TS: ' + new Date(d.ts * 1000).toLocaleTimeString('ru-RU'); xe_src.textContent = d.source || 'xe'; xe_link.href = d.url || '#'; window.__lastXePrice = next;
        }
    } catch { err.style.display = ''; err.textContent = 'Ошибка сети/парсинга XE'; xe_spread_bin.textContent = '—'; xe_spread_byb.textContent = '—'; window.__lastXePrice = null; }
...
function applyGF() { refreshGFNow(); const pr = currentGfPair(); if (pr) history.replaceState(null, '', '?' + new URLSearchParams({ ...Object.fromEntries(new URLSearchParams(location.search)), gf_from: pr.from, gf_to: pr.to })); }
async function loadGF() {
    ...
    try {
        const r = await fetch(url); const js = await r.json();
        if (!js.ok) { e.style.display = ''; e.textContent = 'GF ошибка: ' + (js.error || 'unknown'); gf_price.textContent = '—'; gf_ts.textContent = '—'; gf_link.href = '#'; gf_spread_bin.textContent = '—'; gf_spread_byb.textContent = '—'; lastGfPrice = null; }
        else {
            e.style.display = 'none'; const next = js.price; setAnimatedText(gf_price, fmtShort(next) + ' ' + pr.to, lastGfPrice, next);
            gf_ts.textContent = 'TS: ' + new Date(js.ts * 1000).toLocaleTimeString('ru-RU'); gf_link.href = js.url || '#'; lastGfPrice = next;
        }
    } catch { e.style.display = ''; e.textContent = 'Ошибка сети/парсинга GF'; gf_spread_bin.textContent = '—'; gf_spread_byb.textContent = '—'; lastGfPrice = null; }
...
function swapXe() {
    swapValues('xe_from', 'xe_to');
    // сразу применяем и обновляем кросс-курс
    applyXE();
}

function swapGF() {
    swapValues('gf_from', 'gf_to');
    // сразу применяем и обновляем курс
    applyGF();
}
...
function cycleTheme() { const order = ['auto', 'light', 'dark']; const curr = document.documentElement.getAttribute('data-theme') || 'auto'; const next = order[(order.indexOf(curr) + 1) % order.length]; setTheme(next); const btn = document.getElementById('themeBtn'); if (btn) btn.title = `Тема: ${next}`; }
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    fiatEl.addEventListener('change', () => {
        ...
        document.getElementById('xe_to').value = fiatEl.value;
        document.getElementById('gf_to').value = fiatEl.value;
        refreshNow();
    });
